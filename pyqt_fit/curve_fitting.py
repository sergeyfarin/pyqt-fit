...
class CurveFitting(object):
    ...
    :type  fct: callable
    :param fct: Function to optimize. The call will be equivalent to ``fct(p0, xdata, *args)``

    :type  args: tuple
    :param args: Additional arguments for the function

    :type  residuals: callable or None
    :param residuals: Function computing the residuals. The call is equivalent to ``residuals(y,
            fct(x))`` and it should return a ndarray. If None, residuals are simply the
            difference between the computed and expected values.

    :type  fix_params: tuple of int
    :param fix_params: List of indices for the parameters in p0 that shouldn't change

    :type  Dfun: callable
    :param Dfun: Function computing the jacobian of fct w.r.t. the parameters. The call will
            be equivalent to ``Dfun(p0, xdata, *args)``

    :type  Dres: callable
    :param Dres: Function computing the jacobian of the residuals w.r.t. the parameters. The
            call will be equivalent to ``Dres(y, fct(x), DFun(x))``. If None, residuals
            must also be None.

    :type  col_deriv: int
    :param col_deriv: Define if Dfun returns the derivatives by row or column. With n = len(xdata)
            and m = len(p0), the shape of output of Dfun must be (n,m) if 0, and (m,n)
            if non-0.

    :type  constraints: callable
    :param constraints: If not None, this is a function that should always return a list ofvalues
            (the same), to add penalties for bad parameters. The function call is
            equivalent to: ``constraints(p0)``

    :type  lsq_args: tuple
    :param lsq_args: List of unnamed arguments passed to ``optimize.leastsq``, starting with
            ``ftol``

    :type  lsq_kword: dict
    :param lsq_kword: Dictionnary of named arguments passed to py:func:`scipy.optimize.leastsq`,
            starting with ``ftol``

    Once constructed, the following variables contain the result of the fitting:

    :ivar ndarray popt: The solution (or the result of the last iteration for an unsuccessful
        call)
    :ivar ndarray pcov: The estimated covariance of popt.  The diagonals provide the variance
    ...
    .. note::
        ...
        In this implementation, residuals are supposed to be a generalisation of the
        notion of difference. In the end, the mathematical expression of this minimisation is:
